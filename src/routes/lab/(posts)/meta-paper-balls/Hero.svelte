<script lang="ts">
    ...
    onMount(() => {
        ...
        function drawCurve(m: MetaballCurve) {
            ...
            ctx.bezierCurveTo(m.c1out.x, m.c1out.y, m.c2ain.x, m.c2ain.y, m.p2a.x, m.p2a.y);
            ctx.lineTo(m.p2b.x, m.p2b.y);
            ctx.bezierCurveTo(m.c2out.x, m.c2out.y, m.c1bin.x, m.c1bin.y, m.p1b.x, m.p1b.y);
            ctx.closePath();
            ctx.fill();
        }
        ...
        const frame = () => {
            ...
            for (let i = 1; i < drops.length; i++) {
                if (!drops[i].fixed) {
                    const curve = metaball(mainDrop, drops[i], 0.5, drops[i].rad * 3 + mainDrop.rad);
